...
// missing files are downloaded in the foreground (.part -> SHA-256 verify
...
// AutoSync is deliberately opt-in for existing installations. It is an
// enrollment mechanism only: RfidCommon calls it exclusively for cards with
// no normal local RFID assignment, before any legacy playback can start.
...
bool MediaHub_SetAutoSyncEnabled(bool enabled, size_t *written = nullptr);

// Visible storage changes only the MediaHub file layout.  AutoSync always
// uses it, while explicit MediaHub assignments can opt in independently.
bool MediaHub_IsVisibleStorageEnabled();
bool MediaHub_SetVisibleStorageEnabled(bool enabled, size_t *written = nullptr);
bool MediaHub_UseVisibleStorage();
// The visible-storage directory identifier for a configured hub.
String MediaHub_GetHubKey(const String &hostPort);

// Manual maintenance is deliberately limited to the hidden MediaHub cache.
// It never touches /MediaHub or RFID assignments.
...
// of the AutoSync enrollment setting and intentionally never starts playback.
...
struct MediaHubServer {
	...
	String hostPort;
	bool https = false;
	// The stable URL-derived hub key remains the technical identity.  This is
	// only the optional user-facing name for future visible-storage paths.
	String alias;
};

std::vector<MediaHubServer> MediaHub_GetServers();
// Adds a new server, or updates the hostPort/https/alias of an existing one
// with the same name.  Alias must be filesystem-safe and unique when set.
