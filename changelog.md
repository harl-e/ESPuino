...
* 26.09.2026: Add the CLRC663 (SPI breakout) as a third RFID reader type, alongside MFRC522 and PN5180. It is integrated the same way as the existing readers: runtime-selectable in the web interface (General -> RFID reader), auto-detected at boot via the chip's version register, with its own driver task (`src/RfidClrc663.cpp`) following the PN5180 driver's structure - ISO-14443 and ISO-15693 tag reading (including the ICODE-SLIX2 privacy password already configurable for the PN5180, so 'Tonies' work the same way), a configurable removal debounce (`clrc663Debounce`, default 500 ms) and the full "pause when card is removed" / same-card-resume semantics. The chip version is shown in the web interface. Unlike the PN5180, the CLRC663 needs no BUSY pin (wiring: shared SPI pins MOSI/MISO/SCK/CS, SD/shutdown pin to GND) but does not support LPCD - deep-sleep wake-up falls back to the button. Library: [tueddy/CLRC663-Library](https://github.com/tueddy/CLRC663-Library) (same author as the PN5180 library), pinned in `platformio.ini`.

* 25.09.2026: Fix the box looking off but being impossible to wake up after a shutdown during active playback: the ESPuino-side half of the same shutdown race. `AudioPlayer_Exit()` ended with `delete audio`, whose destructor force-deletes the audio decode task (`vTaskDelete` after an unchecked 0.3s mutex timeout) and tears down the I2S channel while that task may still be inside `i2s_channel_write()` - a call which runs outside the mutex `stopSong()` synchronizes on. With playback active the chip could wedge mid-shutdown: LEDs dark, amp muted, but `esp_deep_sleep_start()` never reached, so no wake-up source was armed and the box appeared dead until a power cycle. The delete is pointless anyway - `AudioPlayer_Exit()` runs only from `System_PreparePowerDown()`, i.e. right before deep sleep or restart, where the heap is wiped either way. It is replaced by the synchronized `stopSong()` stop used on every track change, the same abandon-don't-delete pattern `Bluetooth_Exit()` already uses. Complements the audio-library fix from the same day; root-cause analysis in [forum #4724](https://forum.espuino.de/t/absturz-beim-beenden/4724).
